...
${CILIUM_EXEC} \
    cilium-dbg service update --frontend "${LB_VIP}:80" --backends "[${WORKER_IP6}]:80" --k8s-node-port
...
${CILIUM_EXEC} \
    cilium-dbg service update --frontend "[${LB_ALT}]:80" --backends "[${WORKER_IP6}]:80" --k8s-node-port
...
${CILIUM_EXEC} \
    cilium-dbg service update --frontend "[${LB_VIP}]:80" --backends "${WORKER_IP4}:80" --k8s-node-port
...
${CILIUM_EXEC} \
    cilium-dbg service update --frontend "${LB_ALT}:80" --backends "${WORKER_IP4}:80" --k8s-node-port
