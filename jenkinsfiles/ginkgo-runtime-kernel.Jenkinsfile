...
pipeline {
    ...
    environment {
        ...
        BASE_IMAGE="""${sh(
                returnStdout: true,
                script: 'if [ "${run_with_race_detection}" = "" ]; then echo -n "scratch"; else echo -n "quay.io/cilium/cilium-runtime:eb44d35f729e7ad09233d64fd89efbbaca054980@sha256:648ac4150a2142359c723db34029ee02e99516f596cf23748e281741843277a9"; fi'
            )}"""
    }
